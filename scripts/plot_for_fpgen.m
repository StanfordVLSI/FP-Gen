function plot_for_fpgen(filename, bitwidth)
    ...
    TOP_RP = data{strmatch('TOP_RP', headers)};
    ...
    FMA_Mapped_data = filterCellArray(Mapped_data, arch, 'FMA', TOP_RP, 'True');
    CMA_Mapped_data = filterCellArray(Mapped_data, arch, 'CMA', TOP_RP, 'True');
    DW_Mapped_data = filterCellArray(Mapped_data, arch, 'DW_FMA', TOP_RP, 'True');
...
%    title(['Efficiency Frontier for both ', num2str(bitwidth), 'bit FMA and CMA'],'fontsize',18);
  ...
  % dump csv file for frontiers
   ...
   fprintf(fid, 'mm^2/GFLOPS, mW/GFLOPS, %s\n', inputcsv{1});
   for i=1:length(FMA_Mapped_data{3})
       index_in_csv = FMA_Mapped_data{14}(index_FMA{3}(i));
       fprintf(fid, '%f, %f, %s\n', FMA_Mapped_data{3}(i), FMA_Mapped_data{4}(i), inputcsv{index_in_csv});
   end
   fclose(fid);
   ...
   for i=1:length(CMA_Mapped_data{3})
       ...
       fprintf(fid, '%f, %f, %s\n', CMA_Mapped_data{3}(i), CMA_Mapped_data{4}(i), inputcsv{index_in_csv});
   end
   fclose(fid);
   ...
   for i=1:length(DW_Mapped_data{3})
       ...
       fprintf(fid, '%f, %f, %s\n', DW_Mapped_data{3}(i), DW_Mapped_data{4}(i), inputcsv{index_in_csv});
   end
   fclose(fid);
...
%    axis([0 4 10 45]);
    ...
    fprintf(fid, 'Latency, Energy, %s\n', inputcsv{1});
    j=1;
   for i=1:length(FMA_Mapped_data{10})
       index_in_csv = FMA_Mapped_data{14}(index_FMA{9}(i));
       fprintf(fid, '%f, %f, %s\n', FMA_Mapped_data{10}(i), FMA_Mapped_data{9}(i), inputcsv{index_in_csv});
   ...
   for i=1:length(CMA_Mapped_data{10})
       ...
       fprintf(fid, '%f, %f, %s\n', CMA_Mapped_data{10}(i), CMA_Mapped_data{9}(i), inputcsv{index_in_csv});
   ...
   for i=1:length(DW_Mapped_data{10})
       ...
       fprintf(fid, '%f, %f, %s\n', DW_Mapped_data{10}(i), DW_Mapped_data{9}(i), inputcsv{index_in_csv});
       ...
       text(DW_Mapped_data{10}(i),DW_Mapped_data{9}(i), ...
           ['\leftarrow', DW_Mapped_data{13}{index_DW{9}(i)}],...
           'VerticalAlignment','middle',...
           'HorizontalAlignment','left',...
